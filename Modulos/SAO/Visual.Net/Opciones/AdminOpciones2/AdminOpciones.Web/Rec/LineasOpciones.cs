...
using System.Linq;
using System.Web;
using AdminOpciones.Recursos;//no se utiliza??
...
namespace AdminOpciones.Web.Rec
{
    public class LineasOpciones
    {
        ...
        public static string RecOpciones(DataSet _Datos, string Usuario_, int _NumContrato, string _Operacion, DateTime fProceso, int InyectaTuring)
        {
            string _Glosa = "";
            ...
            _Parametros = Acciones.TraeClienteLCR(_Datos, _Operacion);
            ...
            if (_Operacion == "C")
            {
                ...
                if (_MetodologiaRec == 2 | _MetodologiaRec == 3 | _MetodologiaRec == 5)
                {
                    ...
                    _MontoRec = Acciones.EjecutaCalculoRec( _RutRec,
                                                            _CodCliRec,
                                                            _DescClienteRec,
                                                            _ThresholdRec,
                                                            _Rec_metodologia);
                ...
                _Glosa = Acciones.EjecutaProcesoLineasOpciones(_Resultado, _Datos, Usuario_, _NumContrato, _Operacion, InyectaTuring);
            ...
            if (_Operacion == "U")
            {
                ...
                if (_MetodologiaRec == 2 | _MetodologiaRec == 3 | _MetodologiaRec == 5)
                {
                    ...
                    _Glosa = _Result + Acciones.EjecutaProcesoLineasOpciones(_Resultado, _Datos, Usuario_, _NumContrato, _Operacion, InyectaTuring);

                }
                else
                {
                    DataTable _resultado = new DataTable();
                    _resultado = Acciones.EjecutaSP_LineasAnula(_NumContrato, fProceso);

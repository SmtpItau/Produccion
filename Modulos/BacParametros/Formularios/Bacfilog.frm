...
Private Sub txtRut_KeyPress(KeyAscii As Integer)
 If Trim(Txtrut.Text) <> "" And KeyAscii% = vbKeyReturn Then
  ...
  If MISQL.SQL_Fetch(Datos()) <> 0 Then
    MsgBox IIf(optConsulta(0).Value, "Usuario erróneo", IIf(optConsulta(1).Value, "Evento erroneo", "Fecha erronea ")), 16, " Bac-Cambio "
    Exit Sub
  Else
   optOrdena(0).SetFocus
  End If
...
If Trim(Txtrut.Text) = "" Then
 MsgBox "Selección no corresponde, Repita", 16, " Bac-Cambio "
 Txtrut.SetFocus
 Exit Sub
 End If
 ...
 Loop
  If i <> 0 Then
   MsgBox "No se encontraron datos con la selección deseada", 16, " Bac-Cambio "
'   txtFecDesde.Text = Format$(gsBAC_Fecp, gsc_FechaDMA)
'   txtFecHasta.Text = Format$(gsBAC_Fecp, gsc_FechaDMA)
   optFi = ""
   Exit Sub

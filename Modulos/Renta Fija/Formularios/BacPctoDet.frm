...
Begin VB.Form BacPctoDet 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Detalle Operaciones Ventas con Pacto"
   ClientHeight    =   4950
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   12420
   ...
   Begin VB.ComboBox Combo1 
      ...
      ForeColor       =   &H00800000&
      Height          =   315
      ItemData        =   "BacPctoDet.frx":0000
      Left            =   3615
      List            =   "BacPctoDet.frx":000D
      ...
      Begin MSComctlLib.ImageList ImageList1 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "BacPctoDet.frx":0027
               Key             =   ""
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "BacPctoDet.frx":0F01
               Key             =   ""
            EndProperty
         EndProperty
      End
...
Private Sub Combo1_GotFocus()
  Call PROC_POSI_TEXTO(GRILLA, Combo1)
End Sub
...
Private Sub Combo1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        If GRILLA.Col = Col_dcv Then
           GRILLA.TextMatrix(GRILLA.Row, Col_dcv) = Combo1.Text
           GRILLA.TextMatrix(GRILLA.Row, Col_clave_dcv) = IIf(Combo1.Text = "DCV", FUNC_GENERA_CLAVE_DCV, "")
           KeyCode = 13
...
Private Sub Combo1_LostFocus()
    Combo1.Visible = False
    If GRILLA.Col + 1 < GRILLA.cols Then
...
Private Sub Form_Load()
   ...
   Let sMarca = Frm_Vtas_con_Pcto.nMarca
   LblPapel.Caption = Frm_Vtas_con_Pcto.nSerie
   LblCartera.Caption = Frm_Vtas_con_Pcto.sCarteraNorm
   LblTasa.Caption = Frm_Vtas_con_Pcto.dTasaRef
   LblNominalSel.Caption = Frm_Vtas_con_Pcto.dNominal
   LblNominalVender.Caption = Format(Frm_Vtas_con_Pcto.dNominal, FDec4Dec)
   Let bAceptar = False
...
Private Sub Sub_Configurar_Grilla()
   ...
   Let GRILLA.Rows = 2:      Let GRILLA.cols = 17
   ...
   Let GRILLA.TextMatrix(0, Col_Tasa_Venta) = "Tasa Vta.":                 Let GRILLA.ColWidth(Col_Tasa_Venta) = 1000:           Let GRILLA.TextMatrix(1, Col_Tasa_Venta) = Format(0#, FDec0Dec)
   Let GRILLA.TextMatrix(0, Col_vPresente_Venta) = "Valor Tasa Vta. Val.Selec.":   Let GRILLA.ColWidth(Col_vPresente_Venta) = 1000:      Let GRILLA.TextMatrix(1, Col_vPresente_Venta) = Format(0#, FDec0Dec)
   ...
   'PRD-6006 CASS 16-12-2010
   Let GRILLA.TextMatrix(0, Col_dcv) = "Custodia":                            Let GRILLA.ColWidth(Col_dcv) = 2000:                  Let GRILLA.TextMatrix(1, Col_dcv) = ""
   Let GRILLA.TextMatrix(0, Col_clave_dcv) = "Clave":                         Let GRILLA.ColWidth(Col_clave_dcv) = 2000:            Let GRILLA.TextMatrix(1, Col_clave_dcv) = ""
...
Private Sub Sub_Leer_Detalle_Fli()
   ...
   AddParam Envia, Frm_Vtas_con_Pcto.MihWnd
   AddParam Envia, LblPapel.Caption
   AddParam Envia, Frm_Vtas_con_Pcto.sCarteraNormCod
   AddParam Envia, Frm_Vtas_con_Pcto.dRutEmisor
   
'   If miSQL.SQL_Execute("dbo.Sp_Detalle_PACTOS ") <> 0 Then
   If Not Bac_Sql_Execute("SP_DETALLE_PACTOS", Envia) Then
   ...
   Do While Bac_SQL_Fetch(DATOS())
         ...
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_TCmp) = Format(DATOS(8), FDec4Dec)
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_Tasa_Venta) = Format(DATOS(9), FDec4Dec)
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_vPresente_Venta) = Format(DATOS(10), FDec0Dec)
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_vInicial_Venta) = Format(DATOS(11), FDec0Dec)
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_BloqueoPacto) = Format(DATOS(12), FDec0Dec)
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_Marca) = Format(DATOS(13), FDec0Dec)
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_dcv) = DATOS(14)                                    'PRD-6006 CASS 16-12-2010
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_clave_dcv) = DATOS(15)                              'PRD-6006 CASS 16-12-2010
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_Nominal_Venta) = Format(DATOS(7), FDec0Dec)         'PRD-6006 CASS 16-12-2010
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_vPresente_Venta2) = Format(DATOS(10), FDec0Dec)     'PRD-6006 CASS 16-12-2010
         Let GRILLA.TextMatrix(GRILLA.Rows - 1, Col_vInicial_Venta2) = Format(DATOS(11), FDec0Dec)      'PRD-6006 CASS 16-12-2010
         
         Let LblMargen.Caption = DATOS(2)
         Let LblHaircut.Caption = DATOS(3)
...
Private Sub Grilla_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    'PRD-6006 CASS 16-12-2010
    'Se valida que no seleccione la ultima fila donde estan los totales.
    If sMarca <> "P" Or GRILLA.RowSel = GRILLA.Rows - 1 Then
        Exit Sub
    End If
    ...
    If KeyCode = vbKeyReturn Then
            ...
            If GRILLA.ColSel = Col_dcv Then: Let Combo1.Visible = True: Let bPermiteEscribir = True

            'PRD-6006 CASS 16-12-2010
            If GRILLA.ColSel = Col_dcv Then
                    Call PROC_POSI_TEXTO(GRILLA, Combo1)
                    
                    Combo1.Visible = True
                    Combo1.Text = GRILLA.TextMatrix(GRILLA.RowSel, GRILLA.ColSel)
                    Combo1.SetFocus
            
            ElseIf GRILLA.ColSel = Col_Nominal_Venta1 Then
   ...
   If KeyCode = vbKeyReturn Then
      ...
      For I = 1 To GRILLA.Rows - 2
         ...
         If GRILLA.TextMatrix(I, Col_Marca) = "S" Then
            ...
            Let vPresente_Venta2 = GRILLA.TextMatrix(GRILLA.RowSel, Col_vPresente_Venta)
            Let vNominal_Venta1 = GRILLA.TextMatrix(GRILLA.RowSel, Col_Nominal_Venta1)
            ...
            If CDbl(Valor_Valor_Presente) = CDbl(LblNominalVender.Caption) Then
              For A = 1 To GRILLA.Rows - 2
                  If GRILLA.TextMatrix(A, Col_Nominal_Venta1) <> 0 Then
                       Let dDiferencia = (Frm_Vtas_con_Pcto.GRILLA.TextMatrix(Frm_Vtas_con_Pcto.GRILLA.RowSel, 6) - Valor_vPresente_Venta)
...
Private Sub Grabar()
         ...
         Dim sCustodia As String
         Dim sClave As String
         
         If CDbl(LblNominalSel.Caption) > CDbl(Frm_Vtas_con_Pcto.GRILLA.TextMatrix(Frm_Vtas_con_Pcto.GRILLA.RowSel, 3)) Then
         ...
         For nContador = 1 To GRILLA.Rows - 2
            ...
            Let sCustodia = Mid(GRILLA.TextMatrix(nContador, Col_dcv), 1, 1)
            Let sClave = Trim(GRILLA.TextMatrix(nContador, Col_clave_dcv))
            
            AddParam Envia, sCustodia                                                              'Custodia
            AddParam Envia, IIf(sCustodia = "D" And sClave = "", FUNC_GENERA_CLAVE_DCV, sClave)    'Clave Custodia
            
            If Not Bac_Sql_Execute("SP_ACTUALIZA_DETALLE_PACTOS", Envia) Then                      'PRD-6006 CASS 17-12-2010
               Let MousePointer = vbDefault
               Call BacRollBackTransaction
               Call MsgBox("Se ha originado un error en la grabación de detalles Ventas con Pacto." & vbCrLf & vbCrLf & VerSql, vbExclamation, App.Title)
               Exit Sub
            End If
            Frm_Vtas_con_Pcto.bDistribucionManual = True
         Next
         
         If Not BacCommitTransaction Then
              Call MsgBox("Se ha producido un error en la grabación de detalles Ventas con Pacto.", vbExclamation, App.Title)
              Exit Sub
         End If

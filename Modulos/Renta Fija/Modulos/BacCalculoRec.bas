...
' CLIENTE DWT  - ARCHIVO TXT
Public Type ClienteDRV_DWT
    Rut As Long
    Metodologia As Integer
End Type

Public Type Datos_ClienteDRV_DWT
    ClienteDRV_DWT() As ClienteDRV_DWT
End Type
' FIN - CLIENTE DWT  - ARCHIVO TXT
...
Public Sub Proc_Recalculo_Lineas_DRV_DWT(Optional iRut As Long = 0, Optional iCodigo As Long = 0)
    Dim Conexion As ADODB.Connection
    ...
    Dim Parametros As Boolean
    Dim iCadena As String
    Dim CLIENTE As Datos_ClienteDRV_DWT
    Dim Valdatos As Procesos
    ...
    ''Carga de archivo DRV
    
    
    ' Obtiene Clientes traidos desde archivo DW
    Call Proc_Obtiene_ClientesDRV_CALULO_LIN(CLIENTE, iRut, iCodigo)
    ...
    On Error Resume Next
        TotClieDRV = UBound(CLIENTE.ClienteDRV_DWT)
        ErrorTotClieDRV = err.Number
    On Error GoTo 0
    If Not ErrorTotClieDRV = 0 Then
        TotClieDRV = -1
    ...
    For Contador = 0 To TotClieDRV
      Let fRut = CLIENTE.ClienteDRV_DWT(Contador).Rut
      Let fCodigo = 1 'Cliente.Clie_DRV(Contador).Codigo

        
     'Rescata las cartera según rut de Cliente
      Rescata_Cartera_Trading Datos(0), Cartera, Valdatos, largo_vector, fRut, fCodigo
     ...
     'Pesquiza de error en ValDatos del proceso anterior
      If Valdatos.ErrorNumero <> 0 Then
                ...
                Exit Sub
      End If
    ' Valida Operaciones de Cartera vs enviadas por Itau
'    Call Proc_Valida_Operaciones(Cartera, fRut)
...
Public Sub Proc_Obtiene_ClientesDRV_CALULO_LIN(CLIENTE As Datos_ClienteDRV_DWT, Optional iRut As Long = 0, Optional iCodigo As Long = 0)
    Dim Datos()
    Dim indice As Long

    If Not Bac_Sql_Execute("BacTraderSuda..SP_OBTIENE_CLIENTES_DRV") Then
      Exit Sub
    End If

    Do While Bac_SQL_Fetch(Datos())
        ReDim Preserve CLIENTE.ClienteDRV_DWT(indice)
        CLIENTE.ClienteDRV_DWT(indice).Rut = Datos(1)
        CLIENTE.ClienteDRV_DWT(indice).Metodologia = Datos(2)
        indice = indice + 1
    Loop
...
''NUEVO PARA LD1 PRD25609
Public Sub NuevoCalculoLineasDRV(ByVal origen As Integer)
   Dim Datos()
    Dim Resultado As String
    Dim Mensaje As String
    Dim cadena As String
        
     'Carga del archivo DWT_BacLineas.txt
    If Not Proc_Carga_DWT_BacLineas(origen) Then
        Mensaje = "Error en Proceso carga de operaciones DWT_BacLineas, Error en Carga de Archivo"
        Call GRABA_LOG_AUDITORIA(1, gsBac_Fecp, gsBac_IP, gsBac_User, "BTR", "Opc_10100", "01", Mensaje, "", "", " ")
       
    End If
    
    ' Proceso de Calculo de Lineas, SP : sp_ProcesoRecalculoLineas_DRV
     If Not Proc_ReCalculoLineas_DRV() Then
        Mensaje = "Error en Proceso de recalculo de operaciones DWT_BacLineas, Error en Recalculo"
        Call GRABA_LOG_AUDITORIA(1, gsBac_Fecp, gsBac_IP, gsBac_User, "BTR", "Opc_10100", "01", Mensaje, "", "", " ")
...
Public Function Proc_Carga_DWT_BacLineas(ByVal origen As Integer) As Boolean
    ...
    Dim buscarAnterior          As Boolean
    
    Dim seq                     As Integer
    Dim nombreOriginal          As String
 
    Proc_Carga_DWT_BacLineas = False
    buscarAnterior = False
    
    If Right(gsBac_DIRLINEAS, 1) <> "\" Then
      Let gsBac_DIRLINEAS = gsBac_DIRLINEAS & "\"
    End If
    
    Let cNombreArchivo = "DWT_BacLinea_" & Format(gsBac_Feca, "YYYY") & Format(gsBac_Feca, "MM") & Format(gsBac_Feca, "DD") & ".txt"
    nombreOriginal = cNombreArchivo
    Let oPath = gsBac_DIRLINEAS & cNombreArchivo
    
    If Dir(oPath) = "" Then
        Let Screen.MousePointer = vbDefault
        
        ' Para inicio de dia (origen 0):  En caso de que no esté el archivo cargar el ultimo archivo que se haya agregado a la base de datos
        ' Para recalculo (origen 1): Preguntar si quiere procesar el ultimo archivo antes de ejecutar
        Dim mensajeInicio As String
        mensajeInicio = "Problemas al leer interfaz DWT BacLineas, Nombre del archivo no contiene fecha de proceso anterior, el calculo se realizó con el último archivo cargado."
        
        Dim mensajeRecalculo As String
        mensajeRecalculo = "Problemas al leer interfaz DWT BacLineas, Nombre del archivo no contiene fecha de proceso anterior. ¿Desea realizar el cálculo con el último archivo cargado?"
        
        If (origen = 1) Then '' 1 es para recalculo 0 para inicio
            If (MsgBox(mensajeRecalculo, vbQuestion + vbYesNo, App.Title) <> vbYes) Then
                Exit Function
            End If
        Else
            Call MsgBox(mensajeInicio, vbExclamation, App.Title)
    ...
    If Not Bac_Sql_Execute("BacLineas.dbo.sp_EliminarRegistros_DWT_BacLineas", Envia) Then
        Call BacRollBackTransaction
        Let Screen.MousePointer = vbDefault
        Call MsgBox("Error en Proceso carga de operaciones DWT_BacLineas, Error en Eliminacion.", vbExclamation, App.Title)
        
        Exit Function
    End If
    
    ' En caso de que sea cargar archivo anterior, se ejecuta procedimiento almacenado
    If (buscarAnterior) Then
            ' Ejecutar procedimiento que va a buscar archivo y carga
            
             If Not Bac_Sql_Execute("BacLineas.dbo.sp_CargaArchivo_AnteriorDWT", Envia) Then
                Call BacRollBackTransaction
                Call MsgBox("Error en Proceso carga de operaciones DWT_BacLineas, Error en Carga.", vbExclamation, App.Title)
                Exit Function
             End If
             ...
             Do While Not EOF(1)
                ...
                REGISTRO = Trim(xLine)
                 LargoRegistro = Len(xLine)
                    If LargoRegistro <> 81 Then
                    Call BacRollBackTransaction
                    Close #1
                    Call MsgBox("Error en Proceso carga de operaciones DWT_BacLineas, Largo del Registro no es 81.", vbExclamation, App.Title)
                   
                    Exit Function
                End If
                
                 
                'Graba datos a tabla IngresoDWT_BacLineas
                Envia = Array()
                 AddParam Envia, Format(gsBac_Fecp, "YYYY") & Format(gsBac_Fecp, "MM") & Format(gsBac_Fecp, "DD") 'FechaIngreso --> FechaProceso
                 AddParam Envia, seq 'Seq
                 AddParam Envia, REGISTRO 'Registro
                 AddParam Envia, nombreOriginal 'nombreArchivo
                 If Not Bac_Sql_Execute("BacLineas.dbo.sp_IngresoDWT_BacLineas", Envia) Then
                    Close #1
                    Call BacRollBackTransaction
                    Call MsgBox("Error en Proceso Grabacion de operaciones DWT_BacLineas, carga lineas archivo", vbExclamation, App.Title)
    ...
    End If '''-----' fin de if buscarAnterior
        
    ''''Ejecutar procedimiento que separa las operaciones
        If Bac_Sql_Execute("BacLineas.dbo.SP_ConfiguraCampos_lineas") Then
                Proc_Carga_DWT_BacLineas = True
        Else
            Close #1
            Call MsgBox("Error en Proceso Grabacion de operaciones DWT_BacLineas.", vbExclamation, App.Title)
            Proc_Carga_DWT_BacLineas = False
             Call BacRollBackTransaction
            Exit Function
        End If
...
Public Function Proc_ReCalculoLineas_DRV() As Boolean
    Dim Datos()
    Dim Resultado As String
    Dim Mensaje As String
    Dim cadena As String
    Dim indice As Long
    Proc_ReCalculoLineas_DRV = False
    Resultado = ""
    ...
    Call BacBeginTransaction
     ...
     Envia = Array()
     AddParam Envia, Format(gsBac_Fecp, "YYYY") & Format(gsBac_Fecp, "MM") & Format(gsBac_Fecp, "DD") 'FechaIngreso --> FechaProceso
    If Not Bac_Sql_Execute("BacLineas.dbo.sp_ProcesoRecalculoLineas_DRV") Then
        Call BacRollBackTransaction
        Call MsgBox("Error en Proceso Grabacion de operaciones DWT_BacLineas.", vbExclamation, App.Title)
        Call GRABA_LOG_AUDITORIA(1, gsBac_Fecp, gsBac_IP, gsBac_User, "BTR", "Opc_10100", "01", "Nuevo Calculo de Lineas DRV no se ejecutó", "", "", " ")
        Exit Function
    End If
    ...
    Do While Bac_SQL_Fetch(Datos())
        Resultado = Datos(1)
          If (Resultado = "1") Then ' Calculo Exito
              cadena = Datos(1) & "-" & Datos(2) & "-" & Datos(3) & "-" & Datos(4) & "-" & Datos(5) & "-" & Datos(6) & "-" & Datos(7)
              Mensaje = "Calculo Exitoso " & cadena
              Call GRABA_LOG_AUDITORIA(1, gsBac_Fecp, gsBac_IP, gsBac_User, "BTR", "Opc_10100", "01", Mensaje, "", "", " ")
          End If
        indice = indice + 1
    Loop
 Call BacCommitTransaction
    Proc_ReCalculoLineas_DRV = True

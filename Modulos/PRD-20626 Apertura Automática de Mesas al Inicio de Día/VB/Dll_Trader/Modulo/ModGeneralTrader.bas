Attribute VB_Name = "ModGeneralTrader"
...
Public Function Global_Send_Mail(ByVal Desde As String, ByVal Para As String, ByVal Asunto As String, ByVal Cuerpo As String, ByRef oError As String) As Boolean
    On Error Resume Next
    Dim oEnvioMail      As New SendMailCDO

        Global_Send_Mail = False
    
    '-----------------------------------------------------
        oEnvioMail.puerto = "25"
        oEnvioMail.servidor = "172.16.51.10"
    '-----------------------------------------------------
        oEnvioMail.de = "Luis.Guerra.d@Corpbanca.cl" '-> Desde
        oEnvioMail.Para = Para
        oEnvioMail.Asunto = Asunto
        oEnvioMail.Mensaje = Cuerpo
    '-----------------------------------------------------
        If oEnvioMail.Enviar_Correo = False Then
            oError = "Error en el envio de Mail."
        Else
            Global_Send_Mail = True
        End If
    
    Set oEnvioMail = Nothing
    On Error GoTo 0
End Function

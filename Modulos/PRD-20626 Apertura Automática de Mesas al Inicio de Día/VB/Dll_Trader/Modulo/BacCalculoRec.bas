...
Global gsBac_RECAL      As String
Global gsPathFileLog    As String
...
''''''''''       FRM_DETALLE_LCR.lblExpMax = "Total AddON90d:"
...
End Sub
Private Sub Valoriza_Cartera_Trading(Cartera As Negociacion, Datos As Datos_Mercado, Fecha As Date, Optional Numero_Simulacion As Long = 0)
    ...
    Valoriza_Fwd Cartera.Cartera_Fwd, Datos, Fecha, Numero_Simulacion
    ...
    Valoriza_FWD_RF Cartera.Cartera_Fwd_RF, Datos, Fecha, Numero_Simulacion
...
End Sub
Private Sub Calcula_DV01_Principal(Cartera As Negociacion, Datos As Datos_Mercado, Valdatos As Procesos)
    
    'Calcula DV01
    Calcula_DV01 Cartera, Datos, Valdatos
    
End Sub
Private Sub Calcula_DV01(Cartera As Negociacion, Datos As Datos_Mercado, Valdatos As Procesos)
    ...
    For i = 0 To UBound(Datos.Tasas_Fwd)
        For j = 0 To UBound(Datos.Tasas_Fwd(i).Par)
            ...
            DV01_Fwd i, Cartera.Cartera_Fwd, Datos_Aux, Contador, Delta
            Contador = Contador + 1
        Next
    Next
    ...
    For i = 0 To UBound(Datos.Tasas_RF)
        For j = 0 To UBound(Datos.Tasas_RF(i).Par)
            ...
            DV01_FWD_RF i, Cartera.Cartera_Fwd_RF, Datos_Aux, Contador, Delta
            Contador = Contador + 1
        Next
    Next
    ...
    For i = 2 To UBound(Datos.TC)
        ...
        'Calcula el impacto en la cartera forward
        DV01_Fwd_Mon i, Cartera.Cartera_Fwd, Datos_Aux, Contador, Datos.Paridad(i) * Delta
        'Calcula el impacto en la cartera de fwd renta fija
        DV01_FWD_RF_Mon i, Cartera.Cartera_Fwd_RF, Datos_Aux, Contador, Datos.Paridad(i) * Delta
...
End Sub
Private Sub DV01_Fwd(Codigo_Curva As Long, Operacion() As Operaciones_Fwd, Datos As Datos_Mercado, Iteracion As Long, Delta As Double)
    ...
    If Error = 0 Then
        For k = 0 To limk
            ...
            If Codigo_Curva = Operacion(k).Codigo_descuento(0) Or Codigo_Curva = Operacion(k).Codigo_descuento(1) Then
                Operacion(k).Valor_Simulacion(Iteracion) = (MtM_Fwd(Operacion(k), Datos, Datos.Fecha) - Operacion(k).Valor_Mercado) / Delta
            End If
        Next
    End If
    
End Sub
Private Sub DV01_Fwd_Mon(Codigo_Moneda As Long, Operacion() As Operaciones_Fwd, Datos As Datos_Mercado, Iteracion As Long, Delta As Double)
...
End Sub
Private Sub DV01_FWD_RF(Codigo_Curva As Long, Operacion() As Operaciones_FWD_RF, Datos As Datos_Mercado, Iteracion As Long, Delta As Double)
    ...
    If Error = 0 Then
        For k = 0 To limk
            ...
            If Codigo_Curva = Operacion(k).Cod_Tasa Or Codigo_Curva = Operacion(k).Cod_Tasa_F Then
                Operacion(k).Valor_Simulacion(Iteracion) = (MtM_FWD_RF(Operacion(k), Datos, Datos.Fecha) - Operacion(k).Valor_Mercado) / Delta
            End If
        Next
    End If
    
End Sub
Private Sub DV01_FWD_RF_Mon(Codigo_Moneda As Long, Operacion() As Operaciones_FWD_RF, Datos As Datos_Mercado, Iteracion As Long, Delta As Double)
...
End Sub
Private Sub Valoriza_FWD_RF(ByRef Operacion() As Operaciones_FWD_RF, Datos As Datos_Mercado, FechaProc As Date, Optional ByVal XXNumero_Simulacion As Long = 0)
'Funcion que valoriza
    ...
    If Error = 0 Then
        If XXNumero_Simulacion = 0 Then
            For i = 0 To limi
                Operacion(i).Valor_Mercado = MtM_FWD_RF(Operacion(i), Datos, FechaProc)
            Next
        Else
            For i = 0 To limi
                ReDim Preserve Operacion(i).Valor_Simulacion(1 To XXNumero_Simulacion)
                Operacion(i).Valor_Simulacion(XXNumero_Simulacion) = MtM_FWD_RF(Operacion(i), Datos, FechaProc)
            Next
        End If
    End If
    
End Sub
Private Function MtM_FWD_RF(ByRef Operacion As Operaciones_FWD_RF, Datos As Datos_Mercado, FechaProc As Date) As Double
...
Private Function BSAsiatica(ByRef Operacion As Operaciones_Opcion, Datos As Datos_Mercado, Fecha As Date)
    ...
    If Operacion.Plazo > 0 Then
        ...
        CalculaMomentos Operacion, Datos, Fecha, M1, M2, k, Pond
...
End Function
Private Sub CalculaMomentos(Operacion As Operaciones_Opcion, Datos As Datos_Mercado, Fecha As Date, ByRef M1 As Double, ByRef M2 As Double, ByRef k As Double, ByRef Pond As Double)
...
Private Sub Valoriza_Fwd(ByRef Operacion() As Operaciones_Fwd, Datos As Datos_Mercado, FechaProc As Date, Optional ByVal XXNumero_Simulacion As Long = 0)
'Funcion que valoriza los forwards
    ...
    If Error = 0 Then
        ...
        If XXNumero_Simulacion = 0 Then
            For i = 0 To limi
                Operacion(i).Valor_Mercado = MtM_Fwd(Operacion(i), Datos, FechaProc)
            Next
        Else
            For i = 0 To limi
                ReDim Preserve Operacion(i).Valor_Simulacion(1 To XXNumero_Simulacion)
                Operacion(i).Valor_Simulacion(XXNumero_Simulacion) = MtM_Fwd(Operacion(i), Datos, FechaProc)
            Next
        End If
    End If
...
Private Function MtM_Fwd(ByRef Operacion As Operaciones_Fwd, Datos As Datos_Mercado, FechaProc As Date) As Double
'Funcion que valoriza los swaps
    ...
    'Si el flujo no ha sido liquidado se puede valorizar
        ...
        'Seguro de cambio
            ...
            For i = 0 To 1
                Factordesc(i) = (1 + Tasadesc(i) * Operacion.Plazo_efectivo / 360)
            Next
            ...
            'Forward observado que aun no fixea
                For i = 0 To 1
                    ...
                    Factorfwd(i) = (1 + Tasafwd(i) * Operacion.Plazo_fixing / 360)
                Next
        ...
        'Seguro de inflacion
            ...
            Valor(0) = Valor(0) * Operacion.Amortizacion(0) / (1 + Tasadesc(1) * Operacion.Plazo_efectivo / 360)
            'Descuenta el flujo en pesos
            Valor(1) = Operacion.Amortizacion(1) / (1 + Tasadesc(1) * Operacion.Plazo_efectivo / 360)
        
        ElseIf Operacion.Tipo_forward = 2 Then
        'Arbitraje monedas extranjeras
            For i = 0 To 1
                Valor(i) = Datos.TC(Operacion.Moneda(i)) * Operacion.Amortizacion(i) / (1 + Tasadesc(i) * Operacion.Plazo_efectivo / 360)
            Next
   ...
   Screen.MousePointer = vbHourglass
    ...
    If Metodologia = 2 Then
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                MsgError = Valdatos.ErrorSP & " Error " & Valdatos.ErrorDescripcion
                
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 2 Simulaciones). " & MsgError)
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               ...
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 2 Rescata Datos Mercado). " & MsgError)
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               ...
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 2 Rescata Cartera Trading).. " & MsgError)
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 3 Simulaciones). " & MsgError)
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 3 Rescata Datos Mercado).  " & MsgError)
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 3 Rescata Cartera Trading).  " & MsgError)
        ...
        If ClienteTieneDerivados Then
            ...
            If EjecutaBtnREC = True Then
                ...
                If Valdatos.ErrorNumero <> 0 Then
                   'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                    ...
                    Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 3 Carga Completa de Matriz).  " & MsgError)
            ...
            Else
                If Valdatos.ErrorNumero <> 0 Then
                    ...
                    Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 3 Carga Comlleta de Matriz).  " & MsgError)
            ...
            If EjecutaBtnREC = True Then
                ...
                If Valdatos.ErrorNumero <> 0 Then
                   'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                    ...
                    Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 3 Calcula DV01 Principal).  " & MsgError)
            ...
            Calcula_VaR Cartera, MCovar, largo_vector, Datos(0).Fecha, Matriz_DV01, fRut, fCodigo, "EnLinea"
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 5 Simulaciones).  " & MsgError)
        ...
        If EjecutaBtnREC = True Then
            ...
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                ...
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Metodologia = 5 Rescata Cartera Trading).  " & MsgError)
    ...
    If EjecutaBtnREC = False Then
     ...
     If EjecutaBtnREC = True Then
            MsgError = ""
            If Valdatos.ErrorNumero <> 0 Then
               'Call MsgBox("Se ha originado un error. " & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                
                MsgError = "Error en." & Valdatos.ErrorDescripcion
                
                Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proceso Calculo REC : Proceso calculo REC). " & MsgError)
...
Public Sub Proc_Recalculo_Lineas_DRV(Optional iRut As Long = 0, Optional iCodigo As Long = 0)
    ...
    If TotClieDRV = -1 Then
        Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proc_Recalculo_Lineas_DRV) " & "No hay Clientes con Metodologías Netting. ")
    ...
    Else
        ...
        If Parametros = True Then
           'Call MsgBox(iCadena, vbCritical, "Faltan los siguentes parametros")
            ...
            Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, (Proc_Recalculo_Lineas_DRV) " & iCadena & ", Faltan los siguentes parametros")
    ...
    Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, " & " Recalculo Netting Demoró " + Format(Minute(fin) * 60 + Second(fin) - Minute(inicio) * 60 - Second(inicio), "######.##") + " Seg.")
...
Public Sub Proc_Recalculo_LineasCF_DRV(Optional iRut As Long = 0, Optional iCodigo As Long = 0)
    ...
    If TotClieDRV = -1 Then
       'Call MsgBox("No hay Clientes con Metodologías Netting. ", vbInformation, App.Title)
        Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, No hay Clientes con Metodologías Netting. ")
        Exit Sub
    End If
            ...
            If EjecutaBtnREC = True Then
                ...
                If Valdatos.ErrorNumero <> 0 Then
                   'Call MsgBox("Se ha originado un error al tratar de leer Datos ." & Valdatos.ErrorSP & Valdatos.ErrorDescripcion, vbInformation, App.Title)
                    ...
                    Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, " & MsgError)
        ...
        If BacBeginTransaction() Then
              ...
              If Not Lineas_ChequearGrabarRecalculoDRV("BFW", CDbl(1), 1, 1, 0 _
                 ...
                 'MsgBox "Problemas en Procedimientos"
                  Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, Problemas en Procedimientos [Lineas_ChequearGrabarRecalculoDRV]")
...
Public Sub Proc_EnviarMail(Det_MsgError As String, Titulo As String)
    ...
    If Not Bac_Sql_Execute("Execute BacParamSuda.dbo.SP_LEERENVIOMAIL", Envia) Then
        Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, Error en Envio de Mail, Mensaje : " & Det_MsgError & ", " & Titulo)
    Else
        Do While Bac_SQL_Fetch(Datos())
            Let cDesde = cDesde & Trim(Datos(5)) & ";"
             Let cPara = cPara + (Datos(5)) & ";"
        Loop
    End If

    Let cError = ""
    Let cAsunto = "Problemas de Líneas al : " & Format(CDate(gsBac_Fecp), "DD-MM-YYYY")
    Let cCuerpo = "Estimados, " & vbCrLf & vbCrLf & "Adjunto Errores en cálculo de líneas a la fecha : " & Format(CDate(gsBac_Fecp), "DD/MM/YYYY") & _
                                  vbCrLf & vbCrLf & Titulo & vbCrLf & vbCrLf & Det_MsgError & _
                                  vbCrLf & vbCrLf & "Mensaje Generado desde el Proceso de Aperturas Automático"


    If ModGeneralTrader.Global_Send_Mail(cDesde, cPara, cAsunto, cCuerpo, cError) = False Then
        Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, Error en Envio de Mail, Mensaje : " & cError)
    End If
...
''''''''''    Let cPrefijo = ""
''''''''''    If gsSQL_Server <> "CORPSQL05" Then
''''''''''        Let cPrefijo = "Mensaje generado en ambiente de Certificación " & gsSQL_Server & " (Favor No considerar). ..."
''''''''''    End If
''''''''''
...
''''''''''    OutlookMail.Subject = cPrefijo & "Problema Lineas del " & Format(CDate(gsBac_Fecp), "DD-MM-YYYY")
''''''''''
''''''''''    OutlookMail.htmlBody = cPrefijo & vbCrLf & vbCrLf & "<HTML><BODY>" & "Estimados.<br/> <br/>" & _
...
Public Sub Proc_Verifica_Parametros(VerificaSim As String, Prametros As Boolean _
    ...
    If Not Bac_Sql_Execute("BacLineas..SP_RIEFIN_VERIFICA_PARAMETROS_VAR", Envia) Then
        Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, Problemas al Leer Procedimiento. [SP_RIEFIN_VERIFICA_PARAMETROS_VAR]")
...
Private Sub Graba_Matriz_Covarianza_SQL(Covar() As Double, Datos() As Datos_Mercado, Fecha As Date, Valdatos As Procesos)
    ...
    If Not ErrorVar = 0 Then
        Covarianza = -1
        Call WriteLogFile("Mensaje de : ReCalculo de Lineas DRV, Problemas al grabar matriz covarianza en Base de Datos")
...
Private Function WriteLogFile(ByVal sLogEvento As String)
    ...
    Open gsPathFileLog & dFecha & "_LogInicios.log" For Append Access Write Shared As #hFile%

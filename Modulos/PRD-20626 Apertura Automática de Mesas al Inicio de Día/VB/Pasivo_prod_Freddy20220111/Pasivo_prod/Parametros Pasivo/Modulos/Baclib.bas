...
Dim i As Integer
   For i = 0 To 15
        BAC_Parametros.BacParam.StoredProcParam(i) = ""
        BAC_Parametros.BacParam.Formulas(i) = ""
   Next i

   'By Cristián Labarca Rojas 28/Mayo/2001.-
   
   ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   BAC_Parametros.BacParam.WindowParentHandle = BAC_Parametros.hWnd
   BAC_Parametros.BacParam.WindowState = crptMaximized
'   BAC_Parametros.BacParam.WindowState = crptNormal
'   BAC_Parametros.BacParam.WindowBorderStyle = crptFixedDouble
'   BAC_Parametros.BacParam.WindowControlBox = True
'   BAC_Parametros.BacParam.WindowControls = True
'   BAC_Parametros.BacParam.WindowTop = 72
'   BAC_Parametros.BacParam.WindowLeft = 0
'   BAC_Parametros.BacParam.WindowHeight = Screen.Height / Screen.TwipsPerPixelX - 128
'   BAC_Parametros.BacParam.WindowWidth = Screen.Width / Screen.TwipsPerPixelY + 1
...
''''''''''' Sql = "EXECUTE sp_mdvmleervalmon " & nCodMda & ", " & nMes & ", " & nAno
   ...
   If Not BAC_SQL_EXECUTE("sp_mdvmleervalmon", Envia) Then
      ...
      MsgBox "Problemas al leer los valores de moneda", vbInformation
      Exit Function
   ...
   With oControl
      ...
      Do While BAC_SQL_FETCH(Datos())
         ...
         Select Case sTipMnt
         Case "ME"
            .Col = 2: .Text = Format(BacCtrlDesTransMonto(Datos(3)), FDecimal)
            .Col = 3: .Text = Format(BacCtrlDesTransMonto(Datos(4)), FDecimal)
...
'Function BacProxHabil(dFecha As String, plaza As String) As String  ' Douglas Lefin
...
End Sub
'Public Function CalFeriado(cmbMon, objConData, Estado%, Optional Entre As String) As Boolean
'    Dim Var%, Sql1$
'    Dim I As Integer
'Select Case Estado
'
'    Case 3, 30
'
'      ' diferenciar entre moneda local y extranjera
'       '<< SI ESTAMOS COMPRANDO Y FPAGO ENTREGAMOS = CLP
'       '<< SI ESTAMOS COMPRANDO Y FPAGO RECIBIMOS  = USD
'
'       '<< SI ESTAMOS VENDIENDO Y FPAGO ENTREGAMOS = USD
'       '<< SI ESTAMOS VENDIENDO Y FPAGO RECIBIMOS  = CLP
'
'
'        If Entre <> "" Then
'
'
'            If Not IsNumeric(Entre) And Entre <> "Verdadero" And Entre <> "Falso" Then
'
'               Envia = Array()
'               AddParam Envia, Entre
'               If BAC_SQL_EXECUTE("Sp_Busca_Pais_Moneda", Envia) Then
'                  If BAC_SQL_FETCH(datos()) Then
'                     Sql1 = Format(datos(1), "00000")
'
'                  End If
'
'               End If
...
'          objConData.Text = Format(DateAdd("d", 1, objConData.Text), gsc_FechaDMA)
'          'si la moneda <> 'usd
'          If Sql1 <> "00225" And Sql1 <> "00001" Then
'
'            'If Not BacEsHabil(objConData.Text, Sql1) Then   ' douglas lefin
...
'   If Estado > 3 Then
'      MsgBox "Dia Feriado,Busco el Próximo Habil ", 16
'    End If
'    objConData.Text = BacProxHabil(objConData.Text, Sql1)
''End If
...
'
'End Function
'Function BacEsHabil(cFecha As String, plaza As String) As Boolean -- Douglas Lefin
'
'   Dim objFeriado As New clsFeriado
'
'   Dim iAno       As Integer
...
Public Function ValidaRut(Rut As String) As String
   ...
   Multiplo = 2
   Do While Indice > 0
      Suma = Suma + (Val(Mid(Rut, Indice, 1)) * Multiplo)
      Multiplo = Multiplo + 1
      If Multiplo > 7 Then
         Multiplo = 2
      End If
   ...
   Select Case Resto
      Case 10:   Digver = "K"
      Case 11:   Digver = "0"
      Case Else: Digver = Trim(Str(Resto))
   End Select
   ValidaRut = Digver
End Function

Public Function BacValidaRut(Rut As String, Optional dig As String) As Boolean
...
'   End If
   ...
   'BacValidaRut = Trim$(UCase$(Digito))
    
   If Trim$(UCase$(Digito)) = UCase$(Trim$(dig)) Then
      If UCase$(Trim$(dig)) <> "" Then
'         BacValidaRut = Trim$(UCase$(Digito))
         BacValidaRut = True
      Else
         BacValidaRut = True
...
If Int(xCortes) <> xCortes Then
       MsgBox "Fechas NO concuerdan con Períodos Seleccionados", vbExclamation
       xCortes = 0

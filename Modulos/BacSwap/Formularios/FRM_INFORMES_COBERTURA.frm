...
Private Sub FechaCobertura_Change()
   ...
   SQL = ""
   SQL = SQL & "SELECT [N°Cobertura] = CONVERT(CHAR(8),REPLICATE('0', 8 - LEN(nCobertura)) + LTRIM(nCobertura)) "
   SQL = SQL & ",      [Modulo]      = cModulo "
   SQL = SQL & ",      [N° Derivado] = CONVERT(CHAR(8),REPLICATE('0', 8 - LTRIM(LEN(nDerivado))) + LTRIM(nDerivado)) + '-' + LTRIM(nCorrela) "
   SQL = SQL & "FROM   BacTraderSuda..COBERTURAS "
   SQL = SQL & "ORDER BY nCobertura , nDerivado , nCorrela "
...
Private Sub Informe_Coberturas(MiDestion As DestinationConstants, MiCobertura As Double)
   ...
   Dim lngMiCobertura As Long
   lngMiCobertura = CLng(MiCobertura)   'Igualar tipo de dato con el SP para Crystal 8
   
   Call BacLimpiaParamCrw
   
   BACSwap.Crystal.ReportFileName = gsRPT_Path & "Informe_Coberturas.rpt"
           '--> Procedimiento Almacenado : BacTraderSuda..dbo.SP_INFORME_COBERTURAS
   BACSwap.Crystal.WindowTitle = "Informe de Coberturas."
   BACSwap.Crystal.StoredProcParam(0) = lngMiCobertura    'MiCobertura
   BACSwap.Crystal.StoredProcParam(1) = gsBAC_User
   BACSwap.Crystal.Connect = swConeccion
   BACSwap.Crystal.WindowState = crptMaximized
   BACSwap.Crystal.Destination = MiDestion
